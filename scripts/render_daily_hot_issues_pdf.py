...
def markdown_to_html(md: str) -> str:
    ...
    for raw in md.splitlines():
        ...
        if line.startswith("# "):
            ...
            in_ul = _flush_list(out, in_ul)
            issue_no += 1
            out.append(f'<h3 class="issue-title" data-no="{issue_no:02d}">{md_inline(line[4:])}</h3>')
        elif line.startswith("- "):
            if in_lead:
                in_ul = _flush_list(out, in_ul)
            ...
            issue_no += 1
            out.append(f'<h3 class="issue-title" data-no="{issue_no:02d}">{md_inline(text)}</h3>')
        else:
            in_ul = _flush_list(out, in_ul)
            if in_lead:
...
<section class="report-cover">
  ...
  <div class="cover-cards">
    <div class="cover-card"><b>ORIGINAL SOURCE FIRST</b><span>제목/반응보다 원문 내용·공식 근거 중심</span></div>
    <div class="cover-card"><b>RELATIONSHIP GRAPH</b><span>기업·인물·OmOCon/X·기회 소스 관계 추적</span></div>
    <div class="cover-card"><b>ACTION BRIEF</b><span>Jinwang 관점의 실행/감시 포인트 분리</span></div>
  </div>
</section>
