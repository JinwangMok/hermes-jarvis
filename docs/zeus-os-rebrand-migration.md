...
**Date:** 2026-05-07  
**Scope:** product and architecture identity for `jinwang-jarvis` becoming ZeusOS without a big-bang rename

## Executive decision

`jinwang-jarvis` should evolve into **ZeusOS** as the product/control-plane identity, but the first migration step is **not** a repository, package, import, systemd, wiki, or database rename.
...
| **Jarvis** | Personal-intelligence capability pack: mail, calendar, briefings, hot issues, opportunity radar, HOOO/Houroboros | Retained as a capability-pack and legacy compatibility name |
...
3. **Jarvis is not erased.** It becomes the personal-intelligence capability pack inside ZeusOS.
...
- `src/jinwang_jarvis/` package rename;
- `python -m jinwang_jarvis.cli` removal;
...
| Python distribution `jinwang-jarvis` | Keep | Plugin requirements and installs may depend on it |
| Import namespace `jinwang_jarvis` | Keep | Tests, plugins, docs, and scripts depend on it |
| CLI `python -m jinwang_jarvis.cli` | Keep | Existing docs/systemd/skills use it |
| Zeus subcommand | Keep and document | `python -m jinwang_jarvis.cli zeus ...` is the current safe ZeusOS entrypoint |
| systemd units `jinwang-jarvis-*` | Keep | Renaming can create ghost timers or duplicate schedules |
| `hermes-gateway.service` | Never rename as ZeusOS | It is the Hermes host/gateway boundary |
| Wiki `queries/jinwang-jarvis-*` paths | Keep until writer migration | Generated reports and indexes rely on stable paths |
...
- create a separate `jarvis` Hermes profile for mail, calendar, news, and recurring report delivery;
- keep KARVIS, contractor, and voice profiles as later candidates after the Jarvis split is proven;
...
This profile split must remain compatibility-first: it may add a Jarvis-owned profile/gateway deployment path, but it must not rename the repository, Python package, existing CLI, systemd units, wiki paths, or Hermes source in the first PR.
...
3. **Zeus CLI parser drift check:** remove or regression-test duplicated parser definitions between `src/jinwang_jarvis/cli.py` and `src/jinwang_jarvis/zeus_os/cli.py`.
4. **Path-hardcoding audit:** remove unsafe `/home/jinwang/workspace/jinwang-jarvis` assumptions from tests/defaults where feasible.
...
```bash
PYTHONPATH=src python3 -m jinwang_jarvis.cli zeus init
PYTHONPATH=src python3 -m jinwang_jarvis.cli collect-mail --config config/pipeline.local.yaml
```

ZeusOS is therefore the new product/control-plane identity, while `jinwang-jarvis` and `jinwang_jarvis` remain compatibility surfaces during the transition.
