# ZeusOS Minerva Workflow

Minerva is a ZeusOS-native, Hermes-source-untouched workflow harness for turning a Discord-origin request into a task-specific operating thread, interactive interview state, immutable seed, explicit execution handoff, evaluation, drift visibility, and an evolution proposal.
...
- `state/minerva.db`
- `data/minerva/<run_id>/origin.json`
- `data/minerva/<run_id>/thread_handoff.json` when thread creation is pending
- `data/minerva/<run_id>/interview_state.json`
- `data/minerva/<run_id>/discord_cards.jsonl`
- `data/minerva/<run_id>/interview.jsonl`
- `data/minerva/<run_id>/seed.json`
- `data/minerva/<run_id>/seed.md`
- `data/minerva/<run_id>/claude_code_handoff.json` when `run --executor claude-code` is selected
- `data/minerva/<run_id>/execution_log.md`
- `data/minerva/<run_id>/evaluation.md`
- `data/minerva/<run_id>/drift.md`
- `data/minerva/<run_id>/evolution.md`

Thread metadata is stored in `origin.json` and surfaced through `status`/`export`. When Hermes/Boramae finishes a pending handoff, record it with `minerva mark-thread-created --run-id ... --thread-id ...`.
