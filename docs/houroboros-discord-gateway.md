...
This document describes the ZeusOS-owned Hermes plugin bridge in `plugins/hermes_hooo_gateway/`.
...
- Starts a ZeusOS HOOO run with `origin_channel_id=<parent channel>` and `origin_thread_id=<new task thread>`.
...
- Routes button clicks through `HouroborosWorkflow.handle_interaction()` so stale/mismatched interactions are rejected by ZeusOS state rules.
...
1. Symlink/copy plugin into Hermes user plugins, e.g.
   `~/.hermes/plugins/hermes_hooo_gateway -> /home/jinwang/workspace/zeus-os/plugins/hermes_hooo_gateway`.
