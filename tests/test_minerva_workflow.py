...
def _make_seed_ready(workflow: MinervaWorkflow, run_id: str) -> None:
...
def _latest_card(tmp_path: Path, run_id: str) -> dict:
    cards_path = tmp_path / "data" / "minerva" / run_id / "discord_cards.jsonl"
...
def test_minerva_state_machine_creates_artifacts_and_preserves_seed(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    ...
    assert (tmp_path / "data" / "minerva" / run_id / "workflow_design.json").exists()
    ...
    assert (tmp_path / "data" / "minerva" / run_id / "execution_log.md").exists()

    evaluation = workflow.evaluate(run_id)
    assert evaluation["phase"] == "evaluated"
    assert evaluation["passed"] is True
    assert (tmp_path / "data" / "minerva" / run_id / "drift.md").exists()

    evolution = workflow.evolve(run_id)
    assert evolution["phase"] == "evolved"
    assert (tmp_path / "data" / "minerva" / run_id / "evolution.md").exists()
...
def test_minerva_run_blocks_when_workflow_design_gate_fails(tmp_path: Path):
    ...
    design_path = tmp_path / "data" / "minerva" / run_id / "workflow_design.json"
...
def test_minerva_evaluate_blocks_when_placeholder_evidence_missing(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    ...
    workflow.seed(run_id)
    workflow.run(run_id)
    execution_log = tmp_path / "data" / "minerva" / run_id / "execution_log.md"
    execution_log.write_text("# Minerva Execution Log\n\nNo matching acceptance evidence here.\n", encoding="utf-8")

    evaluation = workflow.evaluate(run_id)

    assert evaluation["phase"] == "blocked"
    assert evaluation["passed"] is False
    evaluation_text = (tmp_path / "data" / "minerva" / run_id / "evaluation.md").read_text(encoding="utf-8")
...
def test_minerva_auto_thread_fake_adapter_and_export_metadata(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    ...
    started = workflow.start(
        ...
        auto_open_thread=True,
        thread_name="minerva fake thread",
    ...
    assert exported["status"]["origin"]["thread"]["thread_name"] == "minerva fake thread"
...
def test_minerva_reserves_db_row_before_live_thread_adapter_side_effect(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
...
def test_minerva_start_avoids_same_timestamp_goal_run_id_collision(tmp_path: Path, monkeypatch):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    monkeypatch.setattr(minerva_module, "_now", lambda: "2026-05-02T00:00:00Z")
...
def test_minerva_pending_thread_handoff_and_mark_created(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    ...
    started = workflow.start(
        ...
        auto_open_thread=True,
        thread_name="minerva pending thread",
    )
    run_id = started["run_id"]

    assert started["origin"]["thread_id"] is None
    assert started["origin"]["thread"]["state"] == "pending"
    handoff_path = tmp_path / "data" / "minerva" / run_id / "thread_handoff.json"
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    assert handoff["action"] == "discord.create_thread"
    assert handoff["request"]["thread_name"] == "minerva pending thread"
...
def test_minerva_cli_namespace_emits_json_and_exports(tmp_path: Path, capsys):
    config_file = _write_config(tmp_path)
    ...
    assert main(["minerva", "turn", "--config", str(config_file), "--run-id", run_id, "--message", "Need seed first"]) == 0
    ...
    for message in [
        ...
        assert main(["minerva", "turn", "--config", str(config_file), "--run-id", run_id, "--message", message]) == 0
        _payload(capsys)

    for command, expected_phase in [("seed", "seeded"), ("run", "running"), ("evaluate", "evaluated"), ("evolve", "evolved")]:
        assert main(["minerva", command, "--config", str(config_file), "--run-id", run_id]) == 0
        assert _payload(capsys)["phase"] == expected_phase

    assert main(["minerva", "status", "--config", str(config_file), "--run-id", run_id]) == 0
    status = _payload(capsys)
    assert status["phase"] == "evolved"
    assert status["origin"]["thread_id"] == "thread-1"
    assert "seed.md" in " ".join(status["artifacts"])

    assert main(["minerva", "export", "--config", str(config_file), "--run-id", run_id]) == 0
    exported = _payload(capsys)
    assert exported["run"]["goal"] == "Ship ZeusOS-native workflow"


def test_minerva_cli_minerva_alias_pending_thread_and_mark_created(tmp_path: Path, capsys):
    config_file = _write_config(tmp_path)
...
def test_minerva_invalid_phase_transitions_are_rejected(tmp_path: Path, capsys):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    ...
    for command in ("run", "evaluate", "evolve"):
        assert main(["minerva", command, "--config", str(config_file), "--run-id", run_id]) == 1
        payload = _payload(capsys)
        assert payload["ok"] is False
        assert "Invalid minerva transition" in payload["error"]

    _make_seed_ready(workflow, run_id)
    assert main(["minerva", "seed", "--config", str(config_file), "--run-id", run_id]) == 0
    _payload(capsys)
    assert main(["minerva", "evolve", "--config", str(config_file), "--run-id", run_id]) == 1
    payload = _payload(capsys)
    assert "cannot evolve from phase seeded" in payload["error"]


def test_minerva_alias_and_skill_docs_exist():
    root = Path(__file__).resolve().parents[1]
    minerva_skill = root / "skills" / "minerva" / "SKILL.md"
    minerva_skill = root / "skills" / "minerva" / "SKILL.md"

    assert minerva_skill.exists()
    assert minerva_skill.exists()
    combined = minerva_skill.read_text(encoding="utf-8") + "\n" + minerva_skill.read_text(encoding="utf-8")
...
def test_minerva_load_run_reports_missing_run(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
...
def test_minerva_discord_interview_card_and_ambiguity_gate(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)

    started = workflow.start(goal="Design Discord-native Minerva", origin_platform="discord", origin_channel_id="parent", auto_open_thread=True)
    run_id = started["run_id"]

    state_path = tmp_path / "data" / "minerva" / run_id / "interview_state.json"
    cards_path = tmp_path / "data" / "minerva" / run_id / "discord_cards.jsonl"
...
def test_minerva_interview_choices_reduce_ambiguity_and_seed_records_decisions(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    started = workflow.start(goal="Implement interactive Minerva", origin_platform="discord", origin_channel_id="parent")
    run_id = started["run_id"]

    workflow.turn(run_id, "Scope: ZeusOS-owned Minerva only")
...
def test_minerva_claude_code_executor_writes_handoff_without_running_claude(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    ...
    handoff_path = tmp_path / "data" / "minerva" / run_id / "claude_code_handoff.json"
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
...
def test_minerva_freeform_turn_does_not_bypass_ambiguity_gate(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
...
def test_minerva_other_opinion_routes_dimension_without_resolving_all(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
...
def test_minerva_other_opinion_plain_reply_resolves_only_pending_dimension(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
...
def test_minerva_seed_requires_all_dimensions_even_at_threshold(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
...
def test_minerva_auto_open_from_existing_thread_requests_sibling_thread(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    ...
    handoff = json.loads((tmp_path / "data" / "minerva" / run_id / "thread_handoff.json").read_text(encoding="utf-8"))
...
def test_minerva_redacts_secret_like_turns_before_artifacts(tmp_path: Path):
    ...
    run_dir = tmp_path / "data" / "minerva" / run_id
...
def test_minerva_rejects_path_traversal_run_id(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    ...
    try:
        ...
        assert "Invalid minerva run_id" in str(exc)
    else:  # pragma: no cover
        raise AssertionError("path traversal run_id must be rejected before file access")



def test_minerva_discord_card_contract_v2_components_and_custom_ids(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    started = workflow.start(
    ...
    assert card["card_id"] == f"minerva-interview:{run_id}"
    assert card["card_revision"] == 1
    assert card["idempotency_key"] == f"minerva:discord-card:{run_id}:1"
    ...
    assert all(component["custom_id"].startswith("minerva:v2:") for component in components)
...
def test_minerva_interaction_reducer_continue_writes_new_card_and_rejects_stale(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    ...
    log_path = tmp_path / "data" / "minerva" / run_id / "interaction_log.jsonl"
    ...
    try:
        ...
        assert "Stale or unknown Minerva interaction card revision" in str(exc)
    else:  # pragma: no cover
        raise AssertionError("stale Discord button click must be rejected")


def test_minerva_interaction_propose_seed_rechecks_ambiguity_gate(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    ...
    _make_seed_ready(workflow, run_id)
    latest_revision = len((tmp_path / "data" / "minerva" / run_id / "discord_cards.jsonl").read_text(encoding="utf-8").splitlines())
...
def test_minerva_interaction_custom_id_validation_and_origin_mismatch(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    started = workflow.start(goal="Validate custom id", origin_platform="discord", origin_channel_id="parent")
    run_id = started["run_id"]
    card = json.loads((tmp_path / "data" / "minerva" / run_id / "discord_cards.jsonl").read_text(encoding="utf-8").splitlines()[-1])
    ...
    try:
        workflow.handle_interaction(run_id, "select_proposal", custom_id=f"minerva:v2:select_proposal:other-run:r1:dscope:oa", origin_channel_id="parent")
    except ValueError as exc:
...
def test_minerva_cli_interact_reducer_emits_json(tmp_path: Path, capsys):
    config_file = _write_config(tmp_path)
    assert main([
        "minerva", "start", "--config", str(config_file), "--goal", "CLI button reducer", "--origin-platform", "discord", "--origin-channel-id", "parent"
    ]) == 0
    started = _payload(capsys)
    run_id = started["run_id"]

    assert main([
        "minerva", "interact", "--config", str(config_file), "--run-id", run_id, "--action", "continue_interview", "--card-revision", "1", "--origin-channel-id", "parent", "--actor-id", "u1"
    ]) == 0
    payload = _payload(capsys)
    ...
    assert main([
        "minerva", "interact", "--config", str(config_file), "--run-id", run_id, "--action", "propose_seed", "--card-revision", "2", "--origin-channel-id", "parent"
    ]) == 1
...
def test_minerva_interaction_reducer_phase_and_thread_guards(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
    ...
    try:
        ...
        assert "Invalid Minerva interaction continue_interview from phase seeded" in str(exc)
    else:  # pragma: no cover
    ...
    try:
        ...
        assert "Invalid Minerva interaction cancel from phase seeded" in str(exc)
    else:  # pragma: no cover
        raise AssertionError("cancel must be rejected after seeding")


def test_minerva_records_alignment_checkpoint_for_every_user_turn(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
...
def test_minerva_alignment_checkpoint_is_visible_in_discord_card_and_seed(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
...
def test_minerva_proposal_selection_records_alignment_checkpoint(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
...
def test_minerva_seed_requires_alignment_checkpoint_not_just_resolved_dimensions(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
...
def test_minerva_non_proposal_buttons_record_alignment_checkpoints(tmp_path: Path):
    config_file = _write_config(tmp_path)
    workflow = MinervaWorkflow.from_config_path(config_file)
