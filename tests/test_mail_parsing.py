...
def test_derive_roles_distinguishes_sent_direct_cc_and_broadcast():
    ...
    assert _derive_interaction_role(folder_kind="inbox", subject="Status report 요청", from_addr="boss@example.com", to_addrs=["me@example.com"], cc_addrs=[], self_addresses=self_addresses) == "status-request"
    assert _derive_interaction_role(folder_kind="inbox", subject="결정 부탁드립니다", from_addr="boss@example.com", to_addrs=["me@example.com"], cc_addrs=[], self_addresses=self_addresses) == "decision-request"
    assert _derive_interaction_role(folder_kind="inbox", subject="진행 상황 공유", from_addr="peer@example.com", to_addrs=["team@example.com"], cc_addrs=["me@example.com"], self_addresses=self_addresses) == "cc-for-awareness"
    assert _derive_interaction_role(folder_kind="inbox", subject="Re: weekly team update", from_addr="peer@example.com", to_addrs=["team@example.com"], cc_addrs=[], self_addresses=self_addresses) == "team-thread-update"
