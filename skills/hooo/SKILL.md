---
name: hooo
description: ZeusOS Houroboros workflow harness for Discord-origin Interview -> Seed -> Execute -> Evaluate -> Evolve runs.
metadata:
  category: software-development
  locale: ko-KR
---

# /hooo — ZeusOS Houroboros Workflow Harness

Use this skill when the user invokes `/hooo` or asks for an Ouroboros-style ZeusOS workflow. Treat `/hooo` as a full workflow harness, not as interview-only UX.
...
```bash
PYTHONPATH=src python3 -m zeus_os.cli hooo start --config config/pipeline.local.yaml --goal "..." --origin-platform discord --origin-channel-id "..." --origin-message-id "..." --auto-open-thread --thread-name "..."
PYTHONPATH=src python3 -m zeus_os.cli hooo mark-thread-created --config config/pipeline.local.yaml --run-id "..." --thread-id "..." --jump-url "..."
PYTHONPATH=src python3 -m zeus_os.cli hooo turn --config config/pipeline.local.yaml --run-id "..." --message "Scope: ..."
PYTHONPATH=src python3 -m zeus_os.cli hooo turn --config config/pipeline.local.yaml --run-id "..." --message "Acceptance: ..."
PYTHONPATH=src python3 -m zeus_os.cli hooo turn --config config/pipeline.local.yaml --run-id "..." --message "Constraint: ..."
PYTHONPATH=src python3 -m zeus_os.cli hooo turn --config config/pipeline.local.yaml --run-id "..." --message "Executor: claude-code"
PYTHONPATH=src python3 -m zeus_os.cli hooo turn --config config/pipeline.local.yaml --run-id "..." --message "Permission: seed approved"
PYTHONPATH=src python3 -m zeus_os.cli hooo seed --config config/pipeline.local.yaml --run-id "..."
PYTHONPATH=src python3 -m zeus_os.cli hooo run --config config/pipeline.local.yaml --run-id "..." --executor claude-code
PYTHONPATH=src python3 -m zeus_os.cli hooo evaluate --config config/pipeline.local.yaml --run-id "..."
PYTHONPATH=src python3 -m zeus_os.cli hooo evolve --config config/pipeline.local.yaml --run-id "..."
PYTHONPATH=src python3 -m zeus_os.cli hooo status --config config/pipeline.local.yaml --run-id "..."
```
...
- Live smoke should validate the real Discord sibling-thread side effect plus ZeusOS card/reducer/seed path. If the tooling cannot impersonate a human Discord UI click, say so and treat `hooo interact --custom-id ...` as API-equivalent reducer validation, not as a physical UI click.
- Verification should include `compileall`, targeted HOOO/CLI regressions, full `pytest -q` with enough timeout for the ZeusOS suite, and a CLI smoke that checks `interview_state.json`, `discord_cards.jsonl`, `thread_handoff.json`, `seed.json`, and `claude_code_handoff.json`.
...
- For Minerva/HOOO directional scope corrections during ZeusOS rearchitecture, use `references/minerva-zeusos-scope-correction-2026-05-09.md`: the mission remains the whole ZeusOS Agent OS platform unless Jinwang explicitly narrows it; every leaf should realign to that mission before implementation.
