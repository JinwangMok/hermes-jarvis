...
Use this reference when continuing ZeusOS OS-style repository rearchitecture work through Minerva/interview leaves.
...
2. Analogy if useful: e.g. path policy = “지도와 출입 규칙”, registry API = “안전하게 읽는 목록”, Minerva bridge = “legacy minerva와 연결될 계약서”.
...
6. Add Minerva/Minerva compatibility bridge as declaration-only metadata:
   - `legacyRoot: skills`
   - `legacyName: minerva`
...
A reviewer caught a real issue in the Minerva/Minerva runtime-caller leaf: `legacyName` was validated only as a string, then joined as `skills_root / legacy_name`. A manifest like this would escape the `skills/` root:
...
When connecting the Minerva/Minerva bridge to a caller, use a test like:

- construct a temporary ZeusOS repo with `apps/skill-sets/custom-skills/minerva/app.yaml` and `skills/minerva/SKILL.md`;
...
PYTHONPATH=src pytest -q tests/test_declarative_manifests.py::test_compatibility_bridge_legacy_name_cannot_escape_skills_root tests/test_hermes_skill_lifecycle.py::test_audit_hermes_skill_lifecycle_consumes_minerva_minerva_registry_bridge tests/test_hermes_skill_lifecycle.py tests/test_declarative_manifests.py tests/test_paths.py
```

## Search/index bridge evidence pattern

When connecting the Minerva/Minerva bridge to skill search/index:

- write the RED test first: `build_skill_search_index(..., zeus_paths=ZeusPaths(repo_root))` should include a `compatibility_bridge` root for `skills/minerva`;
- build a temp repo with the required declarative roots (`agents`, `agent-shim/hermes`, `channels`, `apps/.../minerva`) and a legacy `skills/minerva/SKILL.md`;
