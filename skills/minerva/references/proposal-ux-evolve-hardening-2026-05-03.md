# Minerva proposal UX evolve hardening — 2026-05-03

Use this reference when hardening or reviewing ZeusOS Minerva proposal-driven interview UX.
...
After a user clicks `Other / new opinion`, Minerva records `pending_freeform_dimension`.
...
For this class of Minerva runtime changes, run:

```bash
python -m compileall -q src/zeus_os tests
PYTHONPATH=src pytest -q tests/test_minerva.py tests/test_minerva_gateway_plugin.py
PYTHONPATH=src pytest -q tests/test_minerva.py tests/test_minerva_gateway_plugin.py tests/test_cli.py tests/test_runtime.py
PYTHONPATH=src pytest -q
```

Known passing evidence from the evolve pass:

- Minerva/gateway targeted: `36 passed`
- broader Minerva/CLI/runtime: `55 passed`
...
- Discord gateway live message-route for normal replies in Minerva threads, so `Other` freeform can be captured without manual CLI `minerva turn`.
