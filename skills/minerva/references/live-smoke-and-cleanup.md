# Minerva live smoke + final cleanup reference

Use this when finishing a `/minerva`/Minerva Discord bridge task after gateway/plugin enablement.
...
2. Start the run with the parent channel and source origin preserved:
   ```bash
   PYTHONPATH=src python3 -m zeus_os.cli minerva start \
...
3. Mark the created live thread:
   ```bash
   PYTHONPATH=src python3 -m zeus_os.cli minerva mark-thread-created \
     --config config/pipeline.local.yaml \
     --run-id "$RUN_ID" \
     ...
     --jump-url "$JUMP_URL"
   ```
4. Verify `discord_cards.jsonl`; extract a real `custom_id`; reduce it through `minerva interact`.
5. Add structured turns for `Scope:`, `Acceptance:`, `Constraint:`, `Executor:`, and `Permission:`. Then run `minerva seed` and confirm `seed_ready=true`, low ambiguity, and phase `seeded`.
...
```bash
PYTHONPATH=src python -m compileall -q src/zeus_os plugins/hermes_minerva_gateway tests/test_minerva.py tests/test_minerva_gateway_plugin.py
PYTHONPATH=src pytest -q tests/test_minerva.py tests/test_minerva_gateway_plugin.py tests/test_hermes_skill_context.py tests/test_hermes_skill_search.py
...
print('minerva_link=', os.path.realpath(Path.home()/'.hermes/plugins/hermes_minerva_gateway'))
PY
...
- Minerva/Minerva harness and Discord bridge: CLI/state machine/plugin/docs/tests/live-smoke evidence.
...
After committing, re-run targeted Minerva/skill tests and confirm both ZeusOS and safe-restart-bundle worktrees are clean.
...
1. Re-read recent Discord messages and session summaries to recover the exact requested scope, especially mixed Minerva + watch-source / information-source work.
...
4. Re-run at least the relevant targeted tests, then the full ZeusOS suite when the changes include workflow/runtime/report/source code:
   ```bash
   PYTHONPATH=src pytest -q tests/test_watch.py tests/test_minerva_gateway_plugin.py tests/test_minerva.py
   PYTHONPATH=src pytest -q
   ```
