---
name: houroboros
description: ZeusOS-native Ouroboros loop skill for continuing Houroboros workflow runs, status, drift, and evolution.
metadata:
  category: software-development
  locale: ko-KR
---

# /houroboros — ZeusOS-Native Ouroboros Loop

Use this skill when the user invokes `/houroboros`, asks to continue a Houroboros run, or asks for status/drift on a ZeusOS workflow run.
...
```bash
PYTHONPATH=src python3 -m zeus_os.cli houroboros start --config config/pipeline.local.yaml --goal "..." --origin-platform discord --origin-channel-id "..." --origin-message-id "..." --auto-open-thread --thread-name "..."
PYTHONPATH=src python3 -m zeus_os.cli houroboros mark-thread-created --config config/pipeline.local.yaml --run-id "..." --thread-id "..." --jump-url "..."
PYTHONPATH=src python3 -m zeus_os.cli houroboros turn --config config/pipeline.local.yaml --run-id "..." --message "Scope: ..."
PYTHONPATH=src python3 -m zeus_os.cli houroboros turn --config config/pipeline.local.yaml --run-id "..." --message "Acceptance: ..."
PYTHONPATH=src python3 -m zeus_os.cli houroboros turn --config config/pipeline.local.yaml --run-id "..." --message "Constraint: ..."
PYTHONPATH=src python3 -m zeus_os.cli houroboros turn --config config/pipeline.local.yaml --run-id "..." --message "Executor: claude-code"
PYTHONPATH=src python3 -m zeus_os.cli houroboros turn --config config/pipeline.local.yaml --run-id "..." --message "Permission: seed approved"
PYTHONPATH=src python3 -m zeus_os.cli houroboros seed --config config/pipeline.local.yaml --run-id "..."
PYTHONPATH=src python3 -m zeus_os.cli houroboros run --config config/pipeline.local.yaml --run-id "..." --executor claude-code
PYTHONPATH=src python3 -m zeus_os.cli houroboros evaluate --config config/pipeline.local.yaml --run-id "..."
PYTHONPATH=src python3 -m zeus_os.cli houroboros evolve --config config/pipeline.local.yaml --run-id "..."
PYTHONPATH=src python3 -m zeus_os.cli houroboros status --config config/pipeline.local.yaml --run-id "..."
PYTHONPATH=src python3 -m zeus_os.cli houroboros export --config config/pipeline.local.yaml --run-id "..."
```
