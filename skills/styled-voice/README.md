...
├── scripts/
│   ├── install.sh                             # config setup helper
...
First store **1-3 short voice samples** into a profile, then write:

```text
/styled-voice [profile=jongwon] 생성할 문장
```
...
- the selected stored profile is treated as `reference_audio`; default profile is `default`
