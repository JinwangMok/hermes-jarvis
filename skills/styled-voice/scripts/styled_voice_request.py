...
try:
    ...
    def collect_profile_audio(library_dir=None, profile=None, **_kwargs):
        ...
        candidates = [base / person / style]
        if style == "default":
            candidates.append(base / person)
        files = []
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--voice", default="default", help="Sample-library profile used when --reference-audio is omitted. '<person>' maps to '<person>/default'.")
    parser.add_argument("--sample-library-dir", default=str(DEFAULT_SAMPLE_LIBRARY_DIR), help="Root directory for stored styled-voice samples")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    sample_library_used = False
    if not reference_audio_paths:
        reference_audio_paths = collect_profile_audio(args.sample_library_dir, args.voice)
        sample_library_used = True
    if not reference_audio_paths:
        raise SystemExit(
        ...
        if direct_ok:
            ...
            if direct_result.success:
                ...
                print(json.dumps({
                    "ok": True,
                    "strategy": "direct",
                    "sample_library_used": sample_library_used,
                    "sample_profile": args.voice if sample_library_used else None,
                    "sample_library_dir": str(Path(args.sample_library_dir).expanduser().resolve()) if sample_library_used else None,
                    "inspections": inspections,
                    "attempts": attempts,
