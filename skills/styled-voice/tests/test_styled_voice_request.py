...
class TestMainFlow:
    ...
    def test_main_uses_sample_library_when_reference_audio_is_omitted(self, monkeypatch, tmp_path, capsys):
        library = tmp_path / "library"
        sample = library / "jongwon" / "default" / "sample.wav"
        ...
        def fake_run_curl_attempt(**kwargs):
            assert kwargs["reference_audio_paths"] == [sample.resolve()]
            kwargs["output_path"].write_bytes(b"RIFF")
            return styled_voice_request.CurlResult(True, 200, "audio/wav", kwargs["output_path"], "direct upload succeeded", kwargs["attempt"])
        ...
        exit_code = styled_voice_request.main([
            "--input", "hello",
            "--voice", "jongwon",
            "--sample-library-dir", str(library),
            "--output-dir", str(tmp_path),
            "--output-name", "result",
        ])
        ...
        captured = capsys.readouterr()
        assert exit_code == 0
        assert '"sample_library_used": true' in captured.out
        assert '"sample_profile": "jongwon"' in captured.out
