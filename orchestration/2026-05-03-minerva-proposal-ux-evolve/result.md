# Minerva proposal UX evolve result

## Scope

Evolved the already-implemented Minerva proposal-driven interview UX after post-implementation review. This is a hardening pass on the Minerva UX axis, not the separate Jarvis information-collector architecture axis.
...
- `PYTHONPATH=src pytest -q tests/test_minerva.py tests/test_minerva_gateway_plugin.py` — `36 passed`.
- `PYTHONPATH=src pytest -q tests/test_minerva.py tests/test_minerva_gateway_plugin.py tests/test_cli.py tests/test_runtime.py` — `55 passed`.
- `PYTHONPATH=src pytest -q` — `244 passed in 124.72s`.

## Remaining evolve backlog

- Discord gateway still needs a live message-route for normal replies in Minerva threads so `Other` freeform can be captured from Discord without manual CLI `minerva turn`.
